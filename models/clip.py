...
from modules.attention_modules import CustomTransformerEncoderLayer
...
class TextEncoder(nn.Module):
    ...
    def __init__(self, vocab_size=50000, embed_dim=512, num_heads=8, num_layers=12, max_seq_len=77, verbose=False):
        super().__init__()
...
class CLIP(nn.Module):
    ...
    def __init__(self, vocab_size=50000, embed_dim=512, temperature=0.07, 
                 text_num_heads=8, text_num_layers=12, max_seq_len=77, verbose=False):
        super().__init__()
        self.verbose = verbose
    ...
    def forward(self, image, text):
        ...
        # logitsは確率分布に変換される前の生のスコア
        
        #logits_per_image[i, j] は i番目の画像とj番目のテキストの類似度
        logits_per_image = similarity_matrix * self.temperature.exp()
        
        #logits_per_text[j,i] は j番目のテキストとi番目の画像の類似度
...
def contrastive_loss(logits_per_image, logits_per_text, verbose=False):
    """
    ...
    if verbose:
        print(f"{'Labels for loss':<30}:{labels}")#ラベルの形状
    ...
    if verbose:
        print(f"{'Image to Text loss':<30}:{loss_i}")#画像からテキストへの損失
    ...
    if verbose:
        print(f"{'Text to Image loss':<30}:{loss_t}")#テキストから画像への損失
...
if __name__ == "__main__":
    ...
    image = torch.randn(2, 3, 224, 224)  # ダミー画像データ [batch_size, channels, height, width]
    
    print("=== Verbose=True デモ ===")
    model = CLIP(verbose=True)
    text_feature=model.encode_text(text)
    image_feature=model.encode_image(image)
    print(f"Text features shape: {text_feature.shape}")
    print(f"Image features shape: {image_feature.shape}")
    ...
    loss = contrastive_loss(image_logits, text_logits, verbose=True)
    print(f"Contrastive loss: {loss}")
    
    print("\n=== Verbose=False デモ ===")
    model_quiet = CLIP(verbose=False)
    text_feature_quiet = model_quiet.encode_text(text)
    image_feature_quiet = model_quiet.encode_image(image)
    image_logits_quiet, text_logits_quiet = model_quiet(image, text)
    loss_quiet = contrastive_loss(image_logits_quiet, text_logits_quiet, verbose=False)
    print(f"Text features shape: {text_feature_quiet.shape}")
    print(f"Image features shape: {image_feature_quiet.shape}")
    print(f"Contrastive loss: {loss_quiet}")
